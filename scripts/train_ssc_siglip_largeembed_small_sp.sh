rm -rf /home/terran/logs/siglip2_base_224_ii_tt_it_hnm_new_sp
# 64k batchsize for 2.048e-3 lr
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 TORCH_CUDNN_V8_API_ENABLED=1 TORCH_DISTRIBUTED_DEBUG=DETAIL torchrun  --standalone --nproc_per_node 4 -m open_clip_train.main \
    --save-frequency 1 \
    --save-most-recent \
    --zeroshot-frequency 1 \
    --dataset-config "configs/datasets/dataset_config_sp.yaml" \
    --lr "1e-5" \
    --beta1 0.9 \
    --beta2 0.95 \
    --warmup 0 \
    ...
    --name 'siglip2_base_224_ii_tt_it_hnm_new_sp' \
    --ssc \
    --siglip_weight 1.0 \
    --ssc_weight 1.0 \
    --grad-checkpointing \
    --resume '/home/terran/logs/epoch_latest.pt'
