...
class CustomTextCLIP(nn.Module):
    ...
    def __init__(
        ...
        probe_size = 197
        out_probe_size = 1
        self.vision_pooling_head_toimage = MultiheadAttentionPoolingHead(
            hidden_size, 
            intermediate_size, 
            num_attention_heads, 
        ...
        self.image_ct_head = nn.Sequential(
            nn.Linear(hidden_size, hidden_size*2),  # First linear layer
            nn.SiLU(),                                     # Activation
            nn.Linear(hidden_size*2, hidden_size*2),  # First linear layer
            nn.SiLU(),                                     # Activation
            nn.Linear(hidden_size*2, embed_dim, bias=True)    # Second linear layer
        )
        self.text_ct_head = nn.Sequential(
            nn.Linear(hidden_size, hidden_size*2),  # First linear layer
            nn.SiLU(),                                     # Activation
            nn.Linear(hidden_size*2, hidden_size*2),  # First linear layer
            nn.SiLU(),                                     # Activation
    ...
    def encode_image(self, image=None, text=None, target_image=None, normalize: bool = True, sample_index=0, average_pool=True, use_ssl_head=False, get_image_text_reconstruction_loss=False):
        ...
        # else:
        #     features = outputs
        # pooler_output, _ = self.visual(image, use_ssl_head=use_ssl_head)
        ...
        if use_ssl_head:
            # output = self.dino_head(last_hidden_state)
        ...
        if get_image_text_reconstruction_loss:
            ...
            if use_ssl_head:
                rc_image_features = self.vision_pooling_head_toimage(pooler_output.unsqueeze(1))
                ...
                rc_text_features = self.vision_pooling_head_totext(pooler_output.unsqueeze(1))
    ...
    def encode_text(self, text=None, image=None, normalize: bool = False, use_ssl_head=True, get_image_text_reconstruction_loss=False):
        ...
        # attention_mask = (text != self.pad_id).to(text)
        # pooler_output, _ = self.text(text, use_ssl_head=use_ssl_head)
