...
from .model_ssc import SSCModel
from .coca_model import CoCa
from .loss import ClipLoss, DistillClipLoss, CoCaLoss, SigLipLoss
from .loss_ssc import SSCLoss
...
def load_checkpoint(
    ...
    # if state_dict["text.token_embedding.weight"].shape[0] == 256000:
    #     del state_dict["text.token_embedding.weight"]
    try:
...
def create_model(
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        if custom_text:
            if "ssc_cfg" in model_cfg:
                model = SSCModel(**model_cfg, cast_dtype=cast_dtype)
...
def create_loss(args):
    if args.distill:
        ...
        return SSCLoss(
            siglip_loss_weight=args.siglip_weight,
            ssc_loss_weight=args.ssc_weight,
            local_loss=args.local_loss,
            rank=args.rank,
            world_size=args.world_size,
        )
...
def create_model_from_pretrained(
    ...
    preprocess = image_transform_v2(
        PreprocessCfg(**model.preprocess_cfg),
        is_train=False,
    )
