...
class TextTransformer(nn.Module):
    ...
    def __init__(
        ...
        # self.ln_final_ssl = norm_layer(width)
        # if proj_bias:
        #     self.text_projection_ssl = nn.Linear(width, output_dim)
        # else:
        #     self.text_projection_ssl = nn.Parameter(torch.empty(width, output_dim))
    ...
    def forward(self, text, use_ssl_head=False):
        ...
        if not use_ssl_head:
            ...
            if self.cls_emb is not None:
                # presence of appended cls embed (CoCa) overrides pool_type, always take last token
                pooled, tokens = text_global_pool(x, pool_type='last')
                pooled = self.ln_final_ssl(pooled)  # final LN applied after pooling in this case
            else:
                x = self.ln_final_ssl(x)
                pooled, tokens = text_global_pool(x, text, pool_type=self.pool_type)

            if self.text_projection_ssl is not None:
                if isinstance(self.text_projection_ssl, nn.Linear):
                    pooled = self.text_projection_ssl(pooled)
                else:
                    pooled = pooled @ self.text_projection_ssl
